...
class MotionHandle:
    ...
    def generatePosFileFromWebotsMotion(self, filename, width=7, precision=1):
        print_width = width         # column spacing
        print_precision = precision # value precision
        ...
        with open(filename, "r") as f:
            ...
            for line in lines:
                if line[0] != "#":
                    joint_vals = [math.degrees(float(val)) for val in line.split(',')[2:]]
                    self.addKeyFrame(40, ChangeNaoJointOrder(joint_vals, 3), description=line.split(',')[1])

        write_filename = (filename.split('\\')[-1].split('.')[0])
        
        with open(write_filename + ".pos", 'w') as posFile:
            posFile.write('  ')
            for joint_name in self.header:
                posFile.write(f'{joint_name:<{print_width}}')
            posFile.write('\n\n')
            ...
            for keyframe in self.keyframes:
                ...
                for i, jointval in enumerate(keyframe["joint_vals"]):
                    posFile.write(f'{jointval:<{print_width}.{print_precision}f}')
                
                posFile.write(f'{keyframe["duration"]}\n\n')
        print(f"{write_filename}.pos generated")

#if __name__ == "__main__":
    #MH = MotionHandle()
    #MH.generatePosFileFromWebotsMotion(r"C:\Users\neera\AppData\Local\Programs\Webots\projects\robots\softbank\nao\motions\Forwards.motion")